...
Sign in first with OAuth or a server key ([Auth](./auth)). Use the signed-in profile context from **`proofable_context`** before create or revoke.
...
| `content` | Secret value stored in Vault. Encrypted at rest; never echoed back via MCP |
...
- Store secrets only through **`proofable_secret_create`**. Confirm the stored name and proof ID. Never print the value in chat.
