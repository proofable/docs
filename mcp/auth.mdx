---
title: MCP auth
description: Pick the auth path for your caller: browser sign-in for interactive clients, a server key for servers, CI, and headless agents.
icon: lock
---
...
For servers, CI, and environments that cannot open a browser, use a durable server key instead. Both paths hit the same endpoint and the same Proofable profile.
