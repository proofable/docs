---
title: MCP auth
description: Host Connect for interactive clients, Profile access keys for servers and automation, and token security.
icon: lock
---
...
For servers, CI, and environments that cannot open a browser, use a durable profile access key instead. Both paths hit the same endpoint and the same Proofable profile.
...
Use a **profile access key** from [Access Keys](https://proofable.me/profile?tab=account) when browser OAuth is not available: servers, CI, cron, containers, cloud agents, and gateways.
...
Marketplace gateways that reserve `Authorization` for their own OAuth (such as Smithery) forward the same key as `x-proofable-access-key`; the hosted MCP maps it onto the same Bearer principal. That is a transport alias, not a second credential: same `npk_*` key, same profile, same revocation path. Leave the gateway's key field empty to Connect with OAuth instead.
