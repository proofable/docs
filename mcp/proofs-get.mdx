...
description: Pull the full proof records behind a wallet or DID, for when a yes or no answer is not enough.
icon: receipt
---

Use when you have an **`identifier`** and need proof records, fields, tags, or delegated context. For a trust action, start with **`proofable_verify_or_guide`**. If the user explicitly needs a yes/no eligibility answer, use **`proofable_proofs_check`** first and call this tool only when details beyond yes/no are needed. Profile totals and account context come from **`proofable_context`**.
