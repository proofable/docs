...
<Note>
  **Default:** add `https://mcp.proofable.me/mcp` and click **Connect**. Optional terminal installer: `npx -y -p @proofable/sdk proofable setup`. Use this page for custom MCP hosts, security review, standalone sign-in, or raw HTTP integration.
</Note>
...
The example below shows the **Proofable SDK CLI** loopback flow (`proofable auth --oauth`). Host MCP clients use the same endpoint with the `client_id` issued to them by DCR and their own loopback `redirect_uri`. Never use `neus-cli` for host clients.
...
| `client_id` | Use |
|-------------|-----|
| `neus-cli` | Proofable SDK CLI (`proofable auth --oauth`): loopback redirect on `127.0.0.1` only |
...
The OAuth examples above show `client_id=neus-cli` because they document the CLI loopback path (`proofable auth --oauth`). Host clients receive their own `client_id` from DCR and send that instead, plus the same `resource=https://mcp.proofable.me/mcp`.
