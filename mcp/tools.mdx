---
title: MCP tools
description: "The complete hosted MCP tool reference: reuse current proofs before creating new ones, then verify, mount agents, and manage secrets."
icon: wrench
---
...
| Update proof metadata | `proofable_proofs_update` | The durable work tracker: set title and `status:`/`workspace:` tags; proofs-get filters by those exact tags. |
| Create or import an agent | `proofable_agent_create` | Links the agent to your profile. Optional separate spend account and controls. Pass an existing account address for dedicated spend. |
...
Pro adds only the four tools needed to run and manage jobs. Product connections, research, model settings, sandbox execution, and private sessions stay in the Proofable app.

| Job | Tool | What it does |
|---|---|---|
| Start work | `proofable_job_create` | Run agent work now or on a schedule. |
| List work | `proofable_job_list` | List jobs available to the signed-in profile. |
| Inspect work | `proofable_job_get` | Read one job and its latest run state. |
| Stop work | `proofable_job_cancel` | Cancel a run or recurring schedule. |
...
| `proofable_proofs_update` | `qHash` (proof ID) | optional `title`, `addTags`, `removeTags` |
...
| Phase | Calls |
|-------|-------|
| Default | Connect → **`proofable_context`** → **`proofable_agent_link`** for agents → **`proofable_verify_or_guide`** before sensitive actions |
...
<CardGroup cols={2}>
  ...
  <Card title="Proofs update" icon="pen" href="./proofs-update">
    Change the title or tags on a proof you own.
  </Card>
