...
description: "Per-tier request limits for the Proofable API, how to read limit headers, and the retry pattern that keeps integrations resilient."
icon: clock
---
...
- **Reuse proofs.** A gate check (1 credit) reads an existing proof. Creating a new one (4 to 73 credits) is heavier and counts against the verification window. Cache the proof ID and check it instead.
...
- **Need higher limits?** Pro raises API throughput four times. Enterprise raises it ten times, with service levels and custom packages. See [Pricing](/pricing).

## 402 and credits

When credits run out, billable calls return **402 Payment Required** with a quote, not 429. Pay for the call with [x402](/gates/pay-per-call), no account needed, or add credits and retry.
