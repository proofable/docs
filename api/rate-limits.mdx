...
description: "What each tier allows, how to read the limit headers, and the retry pattern to use when you hit one."
icon: clock
---
...
- **Reuse proofs.** Checking an existing proof is free. Creating a new one (4 to 73 credits) is heavier and counts against the verification window. Cache the proof ID and check it instead — repeat checks cost nothing.
...
- **Need higher limits?** Pro raises API throughput four times. Enterprise raises it ten times, by agreement. See [Pricing](/pricing).
