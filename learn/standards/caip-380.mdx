...
| `chainId` | number | EVM-only compatibility field for the **account / signing** network (Proofable defaults to Base mainnet, `8453`). Do not put the optional voucher hub here. For NFT, token, or contract checks, set that asset's `chainId` inside `data`. |
