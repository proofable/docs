---
title: "Stop an agent from doing what you did not allow"
sidebarTitle: "Agents"
description: "Set what an agent can access, spend, or change before it acts."
icon: shield-halved
---
...
<Steps>
  <Step title="Give the agent limits">
    [Add Proofable](/mcp/setup), then ask your assistant:

    ```text
    Create or import an agent named draft-agent. Allow draft_email, deny send_email, and require human approval for irreversible actions.
    ```
  </Step>
  <Step title="Load its permission where the tool runs">
    ```bash
    npx -y @proofable/sdk mount draft-agent --apply cursor
    ```
...
| Your tools | The host loads the agent's rules. Start a new session after you mount. |
