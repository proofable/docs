...
description: "Reuse a current proof at the next page, API, or agent action."
icon: rotate
---
...
| An AI assistant | `proofable_verify_or_guide` reuses a proof or returns the next step; `proofable_proofs_check` answers an explicit yes/no question. |
...
Checking a current proof is free on every plan, and signed-in owners checking their own proofs through
MCP pay nothing. You only pay when a new proof has to be created. This is what makes reuse worth
building: the second check, the hundredth check, and a partner's check all cost you nothing.
