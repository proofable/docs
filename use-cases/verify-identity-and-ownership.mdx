...
Define the required checks in your app, use `gateCheck` on your server, and show `VerifyGate` only when something is missing. Publish a listing only for managed commerce. [Gate access](/use-cases/gate-access)

## Share only what's needed

Private by default. Share the proof, not the data. Proofable verifies the underlying evidence privately; your application receives the proof and decision data it needs instead of taking custody of the source identity data.
