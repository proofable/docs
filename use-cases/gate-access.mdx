...
Decide what must already be true, then enforce it where the action happens. One check does not need a gate: [Verify once, read the proof](/verification). Publish a listing for a saved `gateId`, price, or schedule. Field reference: [Gates](/gates).
