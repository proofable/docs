...
<Steps>
  ...
  <Step title="Create or import">
    Call **`proofable_agent_create`** with a stable **`agentId`**. Omit **`agentWallet`** to use your profile, or pass an existing dedicated account address. Follow the returned **`next_action`**.
  </Step>
  <Step title="Confirm">
...
Proofable does not generate or hold private keys. Create a dedicated account in your wallet or key manager, then pass its public address. Store the private key there; Proofable never receives it.
