...
description: "Whichever surface you call, the profile, proofs, and permissions are the same. Pick by who is calling: an MCP client, your app, or your server."
icon: book
---

Pick the surface that matches the caller. The table below maps each one to its endpoint and its sign-in method.

| Surface | Use it for | Endpoint or package | Signs in with |
|---|---|---|---|
| [MCP](/mcp/overview) | Any app, chat, or agent that speaks MCP | `https://mcp.proofable.me/mcp` | OAuth, or an access key |
| [JavaScript SDK](/sdks/javascript) | Apps, backends, and React | `@proofable/sdk` | Hosted sign-in, or an access key on servers |
| [CLI](/sdks/cli) | Terminal setup and agent context | `npx -y @proofable/sdk` | OAuth, or an access key |
