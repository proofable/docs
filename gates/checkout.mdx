...
description: "Sell access with a hosted gate: verify visitors, collect payment, and deliver content in one flow. Buyers keep and reuse their proof."
icon: door-open
---
...
Use the [VerifyGate widget](../widgets/verifygate) or the hosted checkout link for the usual path. This page is the raw server contract.
...
2. **Eligibility**: `GET /api/v1/proofs/check?gateId=...&address=...&includePrivate=true&includeQHashes=true` evaluates the visitor's existing proofs against every requirement. An inline Gate Policy uses the same evaluator over `POST /api/v1/proofs/check` and does not collect listing payment.
...
- Re-run the gate check after every interactive step (OAuth grant, personhood session) before treating checkout as complete. Interactive completions only count once Proofable confirms them here.
...
```json
{
  ...
  "data": {
    ...
    "fulfillment": {
      "delivery": "redirect",
      "type": "redirect_url",
      "value": "https://app.example.com/members"
    }
  }
}
```
...
When the listing unlocks an existing proof, fulfill copies a stored body into the buyer wallet if one exists. A title-only proof (an agent identity, a handle) skips that copy and returns `rewardProof` pointing at the source with `accessGranted: true`. The buyer grant is written either way.
...
<CardGroup cols={2}>
  ...
  <Card title="Pricing" icon="tags" href="/pricing">
    Plans, credits, and who pays
  </Card>
  <Card title="Gates in the SDK" icon="code" href="/sdks/gates">
    getGate, gateCheck, and fulfillGate
  </Card>
